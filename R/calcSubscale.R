...
#' @param forwNames A vector of the items that should be forward-scored.
#' @param revNames A vector of the items that should be reverse scored. Default is NULL in case there are no reverse-scored items.
...
calcSubscale <- function(dataFrame, forwNames, revNames = NULL, revInt = NULL, idxOfSubj = 1){
  ...
  #checks if anything has been supplied to the revNames argument, if it is length greater than zero, and if so whether the first element is something other than NA, in that order
  if(!is.null(revNames) && length(revNames > 0L) && !is.na(revNames[1])){
